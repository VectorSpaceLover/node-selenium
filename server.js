          ...
          for (let index = 0; index < data.length; index++) {
            ...
            switch (value.name) {
              ...
              case siteInfo[1]['name']:
                const availableDays = [];
                if ((personInfo[0][FRIDAY] === 'OK') &&
                (personInfo[1][FRIDAY] === 'OK') &&
                (personInfo[2][FRIDAY] === 'OK')) {
                  availableDays.push(FRIDAY);
                }
                if ((personInfo[0][SATURDAY] === 'OK') &&
                 (personInfo[1][SATURDAY] === 'OK') &&
                  (personInfo[2][SATURDAY] === 'OK')) {
                  availableDays.push(SATURDAY);
                }
                if ((personInfo[0][SUNDAY] === 'OK') &&
                 (personInfo[1][SUNDAY] === 'OK') &&
                  (personInfo[2][SUNDAY] === 'OK')) {
                ...
                const cinema = await cinemaState(value.url);

                for (let aIndex = 0; aIndex < availableDays.length; aIndex++) {
                  const rt = cinema.filter((ele) => {
              ...
              case siteInfo[2]['name']:
                ...
                for (let i = 0; i < availableCinema.length; i++) {
                  for (let j = 0; j < availableDinner.length; j++) {
                    if (availableCinema[i].name === availableDinner[j].name) {
                      const movieList = availableCinema[i].value;
                      for (let p = 0; p < movieList.length; p++) {
                        let cinemaTimeList = movieList[p]['timeList'];
                        const dinnerTimeList = availableDinner[j]['timeList'];
                        let dinner = {};
                ...
                if (availableCinema.length > 0) {
                  ...
                  availableCinema.map((ele) => {
                    ...
                    const arr = ele['value'];
                    arr.forEach((ele) => {
                      const time = ele['timeList'];
                      let outPut;
                      time.forEach((val) => {
                        outPut = 'On ' + day + ' the movie ' +
                        ele['movie'] + ' starts at ' + val['time'] +
                        ' and there is a free table between ' + ele['dinner'];
                        console.log(outPut);
                      });
                    });
                  });
